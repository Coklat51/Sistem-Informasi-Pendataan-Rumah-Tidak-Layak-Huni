<?php  if (!defined('AFISYNTAX')) die('LOE NGAPAIN ANJING!!!!!');
	$aColumns	= array('nama_warga', 'jekel', 'rt', 'rw', 'alamat', 'nama_desa','kd_pondasi', 'kd_kond_atap', 'kd_rangka', 'kd_kond_dinding', 'kd_kond_lantai', 'kd_bahan_atap', 'kd_bahan_dinding', 'kd_bahan_lantai'); 
	$sTable 	  = 'data_warga';
	$join		= 'LEFT JOIN tb_jekel 					ON tb_jekel.kdjekel=data_warga.jekel_warga
					...
					LEFT JOIN aspek_keselamatan b 		   ON data_warga.nik=b.nik ';
	$sLimit 	  = '';
	$sOrder	  = '';
	switch(TRUE){
	...
	switch(TRUE){ 
		case(!empty($aFilteringRules)):
			$sWhere = " WHERE b.kd_pondasi <> 0 AND b.kd_kond_atap <> 0 AND b.kd_rangka <> 0 AND 
							  b.kd_kond_dinding <> 0 AND b.kd_kond_lantai <> 0 AND b.kd_bahan_atap <> 0 AND
							  b.kd_bahan_dinding <> 0 AND b.kd_bahan_lantai <> 0 ".$kode_desa." AND ".implode(" AND ", $aFilteringRules);
		break;
		default:
			$sWhere = " WHERE b.kd_pondasi <> 0 AND b.kd_kond_atap <> 0 AND b.kd_rangka <> 0 AND 
							  b.kd_kond_dinding <> 0 AND b.kd_kond_lantai <> 0 AND b.kd_bahan_atap <> 0 AND
							  b.kd_bahan_dinding <> 0 AND b.kd_bahan_lantai <> 0 ".$kode_desa;
		break;};
	$aQueryColumns = array();
	...
	$output['aaData'][] =  array($no++.'.','<strong>'.$aRow->nikwarga.'</strong>',$aRow->nama_warga,$aRow->jekel,$aRow->rt.'/'.$aRow->rw,$aRow->alamat,$aRow->nama_desa);			
