...
<div class="modal-dialog animated zoomIn" role="document">
	...
	$query	=	$record->ngetoke('linkterkait','*',NULL,'urllinkterkait="'.$record->bener($input['edit']).'"');
echo preg_replace('/\r|\n|[\	]/','','
<div class="modal-dialog animated zoomIn" role="document">
	<form class="modal-content" method="post" action="'.$base['aksi'].md5($base['kunci'].'updatelink').'" enctype="multipart/form-data">
		<div class="modal-header" role="document">
		...
		switch(TRUE){
			case ($query->num_rows > 0):
				...
				if(!empty($data->fotolinkterkait)){ $fotolinkterkait = $base['url'].'/'.$data->fotolinkterkait;} else { $fotolinkterkait = $base['upload']; }
						?><div class="form-group col-sm-12"><?php
							inputText('Url Link','urllinkterkait',$data->urllinkterkait);
						?></div><div class="form-group col-sm-12"><?php
							inputText('Deskripsi Link','deskripsilink',$data->deskripsilink);
						?></div><div class="form-group col-sm-12"><?php
							upFoto('Foto','fotolinkterkait',$fotolinkterkait);
						?></div><?php
...
echo preg_replace('/\r|\n|[\	]/','','
<div class="modal-dialog animated zoomIn"  role="document">
	<form class="modal-content" method="post" action="'.$base['aksi'].md5($base['kunci'].'updatelink').'">
		<div class="modal-header" role="document">
