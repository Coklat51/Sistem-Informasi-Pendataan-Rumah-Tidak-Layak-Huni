...
<div class="modal-dialog animated zoomIn" role="document">
	...
	$query	=	$record->ngetoke('videoedukasi','*',NULL,'linkyoutube="'.$record->bener($input['edit']).'"');
echo preg_replace('/\r|\n|[\	]/','','
<div class="modal-dialog animated zoomIn" role="document">
	<form class="modal-content" method="post" action="'.$base['aksi'].md5($base['kunci'].'updatevideo').'" enctype="multipart/form-data">
		<div class="modal-header" role="document">
			...
			case ($query->num_rows > 0):
				$data	=	$query->fetch_object();
					?><div class="form-group col-sm-12"><?php
						inputText('Deskripsi','deskripsi',$data->deskripsi);
				?></div><div class="form-group col-sm-12"><?php
						inputText('Link Video','linkyoutube',$data->linkyoutube);
					?></div><div class="form-group col-sm-12"><?php
						inputText('Tanggal Video','tanggalvideo',$data->tanggalvideo);
					?></div><?php
...
echo preg_replace('/\r|\n|[\	]/','','
<div class="modal-dialog animated zoomIn"  role="document">
	<form class="modal-content" method="post" action="'.$base['aksi'].md5($base['kunci'].'updatevideo').'">
		<div class="modal-header" role="document">
