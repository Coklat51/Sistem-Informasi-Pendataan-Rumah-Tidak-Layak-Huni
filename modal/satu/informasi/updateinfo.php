...
<div class="modal-dialog animated zoomIn" role="document">
	...
	$query	=	$record->ngetoke('infoberita','*',NULL,'judulberita="'.$record->bener($input['edit']).'"');
echo preg_replace('/\r|\n|[\	]/','','
<div class="modal-dialog animated zoomIn" role="document">
	<form class="modal-content" method="post" action="'.$base['aksi'].md5($base['kunci'].'updateinfo').'" enctype="multipart/form-data">
		<div class="modal-header" role="document">
		...
		switch(TRUE){
			case ($query->num_rows > 0):
				...
				if(!empty($data->fotoberita)){ $fotoberita = $base['url'].'/'.$data->fotoberita;} else { $fotoberita = $base['upload']; }
						?><div class="form-group col-sm-12"><?php
							inputText('Judul Berita','judulberita',$data->judulberita);
						?></div><div class="form-group col-sm-12"><?php
							inputText('Url Berita','urlberita',$data->urlberita);
						?></div><div class="form-group col-sm-12"><?php
							inputText('Tanggal Berita','tglberita',$data->tglberita);
						?></div><div class="form-group col-sm-12"><?php
							upFoto('Foto','fotoberita',$fotoberita);
						?></div><?php
...
echo preg_replace('/\r|\n|[\	]/','','
<div class="modal-dialog animated zoomIn"  role="document">
	<form class="modal-content" method="post" action="'.$base['aksi'].md5($base['kunci'].'updateinfo').'">
		<div class="modal-header" role="document">
